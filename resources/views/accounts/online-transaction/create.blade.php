@extends('layouts.app')
@section('title','Online Transactionss')
@section('css')
    ...
    <div class="col-md-12 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">
                ...
                <form class="forms-sample row" method="POST" action="{{ route('receipts.store') }}" enctype="multipart/form-data">
                    @csrf
                    <div class="col-md-12">
                            ...
                            <div class="col-sm-9">
                                <select name="agent_id" id="agent_id" class="form-control select2">
                                    @foreach($agents as $key => $val)
                                     <option value="{{ $val->id }}">{{ $val->code  }} {{ $val->company_name  }} {{ $val->phone }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input type="text" class="form-control" id="remarks" placeholder="Enter the Remarks" name="remarks" value="{{ old('remarks') }}">
                            </div>
                        </div>
                        <!-- <div class="form-group row"  >
